...
import {deleteProduct, getListProduct} from '../../resource/Product';

const ProductList = ({navigation, route}) => {
  const [list, setList] = useState([]);
  const [title, setTitle] = useState('List Product');
  ...
  const loadData = async () => {
    let response = await getListProduct({}, true);
    if (response) {
      setList(response);
    }
  };
  ...
  // Filter client-side: nama, deskripsi, status
  ...
  const renderItem = (item, index) => {
    return (
      <Pressable
        onPress={() =>
          RootNavigation.navigate('ProductEdit', {item: item})
        }
        key={index}
        style={styles.card}>
      ...
      <View style={styles.container}>
        ...
        {filteredList.length === 0 && (
          <Text style={styles.emptyText}>
            {search ? 'Tidak ditemukan.' : 'Belum ada data product.'}
          </Text>
        )}
        <FlatList
          data={filteredList}
        ...
        />
        {/* Tombol tambah product baru */}
        <TouchableOpacity
          style={styles.fab}
          onPress={() => RootNavigation.navigate('ProductEdit')}>
